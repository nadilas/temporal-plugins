...
func KVManipulator(ctx workflow.Context, key string) (string, error) {
	...
	if err := workflow.ExecuteActivity(ao, kv.KV.Put, key, []byte("value1")).Get(ctx, nil); err != nil {
		return "", err
	}
	var val1 []byte
	...
	var final []byte
	if err := workflow.ExecuteActivity(ao, kv.KV.Get, key).Get(ctx, &final); err != nil {
		return "", err
	}
